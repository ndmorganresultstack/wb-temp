...
export interface SidebarManagerProps {
  ...
  userState: ClientPrincipal | null;
}
// SidebarManager component to handle sidebar logic
export const SidebarManager = ({ isSidebarOpen, toggleSidebar, expandedItem, expandedSubItem, setExpandedItem, 
    setExpandedSubItem, setMenuItem, menuItem, sidebarMinWidth, sidebarMaxWidth, setPageTitle, pageTitle,searchQuery,setSearchQuery, userState }:SidebarManagerProps) => {
      ...
      <nav className="mt-4 mx-[15.5px]">
        <ul>
          {menuItems.map((item:MenuItem) => (
              ...
              <li key={item.name} className={`relative ${expandedItem === item.name && !isSidebarOpen ? 'bg-gray-100 rounded-md' : ''} ${item.name == '' ? 'border-b-1 border-gray-200' : ''}`}>
                <button
                ...
                >                
                  {item.icon && (
                    <item.icon className={`w-[16px] h-[16px] flex-shrink-0 text-gray-600`} />
                  )}
                  {isSidebarOpen && (
                    <>
                ...
                {isSidebarOpen && item.subItems && item.subItems.length > 0 && expandedItem === item.name && (
                      ...
                      <div key={subItem.name}>
                        {subItem.href ? (
                           <Link
                              ...
                              onClick={() => setMenuItem(subItem.href ?? subItem.name)}
                            >
                              {subItem.icon && (
                            ...
                            <button
                              onClick={() => toggleSubAccordion(subItem.name)}
                              className={`block text-sm text-gray-700 py-2 px-2 hover:bg-gray-100 rounded w-[80%] flex items-center
                                ${expandedSubItem === subItem.name && subItem.href == menuItem ? 'font-bold bg-gray-100 border-l-4' : ''}`}
                            >
                              {subItem.icon && (
                              ...
                              {subItem.subItems && subItem.subItems?.length > 0 && (
                                <svg
                                  className={`w-3 h-3 ml-2 transform transition-transform ${
                                    expandedSubItem === subItem.name ? 'rotate-180' : ''
                                  }`}
                                  fill="none"
                                  stroke="rgb(23, 23, 23)"
                                  viewBox="0 0 24 24"
                        ...
                        {isSidebarOpen && subItem.subItems && subItem.subItems.length > 0 && expandedSubItem === subItem.name && (
                          <div className="pl-8 mt-1 space-y-1">
                            {subItem.subItems.map((subSubItem) => (                            
                              <Link
                                key={subSubItem.name}
                                href={subSubItem.href ?? subSubItem.name}
                                className={`block text-xs text-gray-600 py-2 px-2 hover:bg-gray-100 rounded w-[80%] flex items-center
                                  ${menuItem === subSubItem.href ? 'font-bold bg-gray-100 border-l-4' : ''}`}
                                onClick={() => setMenuItem(subSubItem.href ?? subSubItem.name)}
                              >
                                {subSubItem.icon && (
        ...
        <div className={` ${isSidebarOpen ? 'border-1' : ''} rounded p-1 my-10 flex items-center`}>
